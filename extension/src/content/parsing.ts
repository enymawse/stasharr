...
function extractSceneIdFromPathname(pathname: string): string | null {
  const match = pathname.match(/\/scenes\/([^/?#]+)/i);
  return match?.[1] ?? null;
}
...
function extractSceneIdFromCanonical(
  ...
  try {
    ...
    const candidate = extractSceneIdFromPathname(canonicalUrl.pathname);
...
function parseStashDbPage(
  ...
  const sceneIdFromUrl = extractSceneIdFromPathname(url.pathname);
  if (isUuid(sceneIdFromUrl)) {
...
export {
  extractSceneIdFromPathname,
  extractUuidFromPath,
  isUuid,
  parseStashDbPage,
};

export type { StashDbPageParseResult, StashDbPageType };
